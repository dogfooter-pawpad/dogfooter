...
class LYBV4Scene(likeyoubot_scene.LYBScene):
    ...
    def shop_scene(self):
        ...
        if self.status == 0:
            ...
            self.set_option('last_status', self.status)
            self.status = 40
        elif 20 <= self.status < 29:
            self.status += 1
            resource_name = 'shop_scene_list_동료_loc'
            for each in rect_list:
                (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart(
                    self.window_image,
                    resource_name,
    ...
    def monghwan_scene(self):
        ...
        if self.status == 0:
            ...
            resource_name = 'monghwan_scene_' + self.get_game_config(
                lybconstant.LYB_DO_STRING_V4_WORK + 'jido_move_sub_area') + '_loc'
            self.set_option('resource_name', resource_name)
            self.set_option('location_index', 0)
            self.lyb_mouse_drag('monghwan_scene_list_drag_top', 'monghwan_scene_list_drag_bot')
            self.status += 1
    ...
    def monster_josa_scene(self):
        ...
        if self.status == 0:
            ...
            # TODO 마우스 클릭 - 쿵야님 추가
            # self.lyb_mouse_click_location(42, 525)
            # -------------------------------------------
            self.status += 1
    ...
    def jeoljeon_mode_scene(self):
        ...
        if self.status == 0:
            ...
            auto_jamjeryeok_cfg_duration = int(self.get_game_config(lybconstant.LYB_DO_STRING_V4_WORK + 'auto_jamjeryeok_duration'))
            if auto_jamjeryeok_cfg_duration > 0:
                elapsed_time = time.time() - self.game_object.get_scene('main_scene').get_checkpoint('auto_jamjeryeok')
                current_work = self.game_object.get_scene('main_scene').current_work
                if current_work == '자동 사냥' and elapsed_time > auto_jamjeryeok_cfg_duration:
                    self.status = 99998
                    return self.status
    ...
    def main_scene(self):
        ...
        if self.status == 0:
            ...
            if inner_status == 0:
                ...
                self.set_option(self.current_work + '_inner_status', inner_status + 1)
                cfg_auto_jamjeryeok = int(
                    self.get_game_config(lybconstant.LYB_DO_STRING_V4_WORK + 'auto_jamjeryeok_duration'))
